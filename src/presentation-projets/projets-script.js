...
// === ARBRES EN BORDURE (plus ronds et variés) ===
...
// === BUISSONS (sur le sol - bordures uniquement) ===
function createBush(x, z) {
  const bush = new THREE.Mesh(
    new THREE.SphereGeometry(0.6, 16, 16),
    new THREE.MeshPhongMaterial({ color: 0x3d6b2f })
  );
  bush.position.set(x, 0.6, z);
  return bush;
}

// Buissons en bordure inférieure et supérieure
for (let i = 0; i < 8; i++) {
  const x = -12 + i * 3.5;
  scene.add(createBush(x, -9.5 + Math.random() * 0.5));
  scene.add(createBush(x, 9.5 + Math.random() * 0.5));
}

// Buissons en bordure gauche et droite
for (let i = 0; i < 6; i++) {
  const z = -7 + i * 2.5;
  scene.add(createBush(-14 + Math.random() * 0.5, z));
  scene.add(createBush(14 + Math.random() * 0.5, z));
}
...
// --- Fonctions pour créer les textures ---
...
// --- Tente Model ---
class TenteModel {
  constructor(color = 0xff0000, x = 0, y = 0, z = 0, id = "") {
    this.group = new THREE.Group();
    this.group.position.set(x, y, z);
    this.group.userData.tenteId = id;

    const cyl = new THREE.Mesh(
      new THREE.CylinderGeometry(1.25, 1.25, 2, 32),
      new THREE.MeshPhongMaterial({ color })
    );
    this.group.add(cyl);

    const cone = new THREE.Mesh(
      new THREE.ConeGeometry(1.25, 0.8, 32),
      new THREE.MeshPhongMaterial({ color })
    );
    cone.position.y = 1.4;
    this.group.add(cone);
...
class Tente {
  ...
  build() {
    ...
    const stripeTextureBanderoles = this.createStripeTexture(
      256,
      256,
      16,
      c1,
    ...
    // --- Banderoles ---
    const banderoles = new THREE.CylinderGeometry(
      baseRadiusTop * 1.1,
      baseRadiusTop * 1.1,
      0.2,
      32,
...
// --- Remplacer TenteModel par Tente ---
...
new Tente(0xffff00, 0xff00000, 6, 0, -6, "Retro").add(scene);

// --- Village Model complet ---
class VillageModel {
  constructor(x = 0, y = 0, z = 0, id = "") {
    ...
    positions.forEach((pos) => {
      ...
      const invertedRoof = new THREE.Mesh(
        invertedRoofGeometry,
        invertedRoofMaterial
      );
    ...
    // Label village
    const label = createLabel(id || "ID inconnu");
    label.position.y = 5;
    this.group.add(label);
  }
...
function getModalContent(id) {
  const content = {
    ...
    Projets: {
      title: "📋 Projets",
      description:
        "Découvrez nos créations à travers la carte interactive et en 3D de notre village gaulois ! Chaque tente représente un projet unique, accessible en cliquant dessus. Plongez dans notre univers créatif et découvrez comment nous transformons les idées en réalité numérique.\n\n Technologies : Three.js pour la 3D, HTML/CSS/JS pour l'interface, Gestion des événements utilisateur",
    },
    Decathlon: {
      title: "🏃 Decathlon",
      description:
    ...
    "Hidden Snake": {
      title: "🐍 Hidden Snake",
      description:
        " Le joueur doit glisser-déposer les lettres dans le bon ordre pour former le mot \"GAFAM\". Un système d'animation avec retour des lettres en cas d'erreur et confettis en cas de réussite rend le jeu dynamique et challengeant. Ce défi sensibilise aux problématiques des géants de la tech.\n\n Technologies : HTML5 Drag & Drop API, CSS Keyframe Animations, React useEffect pour les timers, Lucide React Icons",
    },
    Retro: {
      title: " 👾 Retro",
      description:
        "Pour créer une expérience nostalgique et unique, nous avons habillé l'intégralité du site dans un style rétro gaming inspiré des consoles 8-bit. Chaque bouton, container et élément visuel rappelle l'ère des jeux vidéo classiques tout en restant moderne et fonctionnel. L'utilisation de la police \"Press Start 2P\" et des animations CSS renforce cette immersion vintage.\n\n Technologies : NES.css, Press Start 2P Font, CSS Animations",
    },
  };
  ...
  return (
    content[id] || {
      title: id,
      description:
        "Bienvenue dans le village ! Cliquez sur les différentes tentes pour découvrir plus d'informations.",
    }
  );
}
...
renderer.domElement.addEventListener("click", (event) => {
  const modal = document.getElementById("modal");
  if (modal.style.display === "block") return; // ignore si modal ouvert

  const distX = event.clientX - dragStart.x;
  const distY = event.clientY - dragStart.y;
  if (Math.sqrt(distX * distX + distY * distY) > dragThreshold) return; // évite drag
  ...
  if (intersects.length > 0) {
    ...
    while (obj && !obj.userData.tenteId && !obj.userData.villageId)
      obj = obj.parent;
    if (obj) openModal(obj.userData.tenteId || obj.userData.villageId);
  }
});

// --- Collision detection ---
const collisionRadius = 0.5; // Rayon de la caméra
const groundSizeX = 15; // Moitié de la taille du sol (30/2)
const groundSizeZ = 10; // Moitié de la taille du sol (20/2)
